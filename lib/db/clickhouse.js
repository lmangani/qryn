...
const samplesReadTable = {
  ...
  _check: async function (tableName) {
    try {
      ...
      samplesReadTable.v1Time = v1EndTime.data.data[0].ts
      logger.warn('!!!WARNING!!! You use cLoki in the backwards compatibility mode! Some requests can be less efficient and cause OOM errors. To finish migration please look here: https://github.com/lmangani/cLoki/wiki/Upgrade')
    } catch (e) {
 ...
 */
const rawStream = async (query) => {
  try {
    const stream = await axios.post(`${getClickhouseUrl()}/?database=${UTILS.DATABASE_NAME()}`,
      query, {
        responseType: 'stream'
      })
    return stream
    ...
    if (e.response) {
      let res = ''
      e.response.data.on('data', d => res += d)
      await new Promise(r => e.response.data.once('end', r))
      throw new Error(res)
    }
    throw e
  }
...
module.exports.rawStream = rawStream
