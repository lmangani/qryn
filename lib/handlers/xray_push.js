/* 
  ...
  Segment API Documentation: https://docs.aws.amazon.com/xray/latest/api/API_PutTraceSegments.html#xray-PutTraceSegments-request-TraceSegmentDocuments
  Example Segment JSON Object:
...
function handler (req, res) {
  ...
  if (streams) {
    if (streams.TraceSegmentDocuments) streams = streams.TraceSegmentDocuments;
    streams.forEach(function (stream) {
      ...
      try {
        try {
          ...
          if (stream.http){
            if (stream.http?.request?.method) JSONLabels.method = stream.stream.http?.request?.method
            if (stream.http?.response?.status) JSONLabels.status = stream.stream.http?.response?.status
          }
          // Sort for fingerprinting
